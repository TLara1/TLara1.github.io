...
\end{equation}
$$\mathbf{y}_k$$ is our signal at time $$k$$ (\ref{eq: signal update equation}) and $$\mathbf{K}_k$$ is the time-_dependent_ Kalman gain, which we will derive shortly. The term $$\mathbf{y}_k-\mathbf{A}\hat{\mathbf{x}}_k'$$ is called the innovation $$\mathbf{i}_k$$, and substituting \ref{eq: signal update equation}, we obtain,
\begin{equation}
